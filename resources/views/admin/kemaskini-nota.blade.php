      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li><a href="{{(url('/kemaskini-senarai-nota-admin', $notasenarai->id))}}">Kemaskini Mesej</a></li>
            <li class="active">Senarai Kemaskini Mesej</li>
          </ol>
        </section>
          ...
          <div class="row">
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                      ...
                      <tbody>
                        <?php
                            $counter = 1;
                         ?>
                         @foreach($notasenarai1 as $key => $value)
                        <tr>
                          <td>{{ $counter }}</td>
                          <td>{{ $value-> mesej}}</td>
                          <td>{{ $value-> sasaran}}</td>
                          <td style="text-align:center;">
                            <a class="btn btn-app" method="DELETE" href="{{ URL('kemaskini-senarai-nota-admin', $value->id) }}" title="Kemaskini" onclick="return confirm('Adakah anda ingin mengemaskini pengguna ini?')"><i class="fa fa-pencil"></i>Kemaskini</a>
                            <a class="btn btn-app" method="DELETE" href="{{ URL('hapus-nota-admin', $value->id) }}" title="Hapus" onclick="return confirm('Adakah anda ingin membuang pengguna ini?')"><i class="fa fa-remove"></i>Hapus</a>
                        </tr>
                        <?php
                            $counter++;
                        ?>
            ...
            <div class="col-md-6">
              <div class="box box-primary">
                <div class="box-header">
                  <h3 class="box-title">Kemaskini Mesej</h3>
                </div>
                <div class="box-body">
                  <form class="form-horizontal" role="form" method="PUT" action="{{ URL('/kemaskini-nota-admin', $notasenarai->id) }}">
                        {!! csrf_field() !!}
                        <div class="form-group{{ $errors->has('mesej') ? ' has-error' : '' }}">
                            <label class="col-md-4 control-label">Mesej</label>
                            <div class="col-md-6">
                                <!-- <textarea name="mesej" value="{{ $notasenarai->mesej}}" id="maklumatP" class="form-control" cols="30" rows="5" required /></textarea> -->
                                <input type='text' name="mesej" value="{{ $notasenarai->mesej}}" id="maklumatP" class="form-control" required />
                        ...
                        <div class="form-group{{ $errors->has('sasaran') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                              <select class="form-control m-bot15" name="sasaran" value="{{ $notasenarai->sasaran}}" id="negeri">
                                  <option hidden> {{ $notasenarai->sasaran}}</option>
                        ...
                        <div class="form-group">
                            <div class="col-md-6 col-md-offset-4">
                                <button type="submit" class="btn btn-primary">
                                    <i class="fa fa-btn fa-pencil"></i> Kemaskini
                                </button>
                            </div>
                        </div>
                      </form>
