@include('sider-admin')
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li><a href="{{ url('kemaskini-profile', $value = Auth::user()->id) }}">Kemaskini Profil</a></li>
              ...
              <div class="nav-tabs-custom">
                ...
                <div class="active tab-pane" id="activity">
                  ...
                  @if (count($errors) > 0)
                    ...
                    <form class="form-horizontal" method="PUT" action= "{{ URL('kemaskiniprofil', $pengguna1->id) }}">
                    ...
                    <div class="form-horizontal">
                      ...
                      <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-10">
                          <button type="submit" id="confirm" class="btn btn-danger">Kemaskini</button>
                        </div>
                      </div>
                    <!-- </form> -->
                  </div>
    ...
    $``````````````````````````````````````````````````````````````````````````````````````````````````````````````(document).ready(function() {
		...
		swal({
			...
			text: "Maklumat "+{{ $pengguna1->name }}+" hendak dihapuskan.",
			type: "warning",
		...
		function(isConfirm){
			if (isConfirm) {
				jQuery.ajax({
					url: '{{ url('kemaskiniprofil', $pengguna1->id) }}',
					data: 'uid=' + uid,
					dataType: 'json',
					type: 'POST',
					success: function (j) {
						if(j == 0){
  						swal("Tidak Berjaya!", "Maklumat "+{{ $pengguna1->name }}+" tidak dapat dihapuskan", "error");
						} else {
							swal({
								title: "Berjaya!",
								text: "Maklumat "+{{ $pengguna1->name }}+" telah dihapuskan.",
								type: "success",
							}
						}
					}
				...
				swal("Dibatalkan", "Maklumat "+{{ $pengguna1->name }}+" tidak dihapuskan", "error");
			}
		});
